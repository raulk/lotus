...
func (n *ProviderNodeAdapter) WaitForPublishDeals(ctx context.Context, publishCid cid.Cid, proposal market.DealProposal, cb storagemarket.OnPublishedCallback) error {
	...
	if err != nil {
		return cb(0, cid.Undef, xerrors.Errorf("WaitForPublishDeals errored: %w", err))
	}
	if receipt.Receipt.ExitCode != exitcode.Ok {
		return cb(0, cid.Undef, xerrors.Errorf("WaitForPublishDeals exit code: %s", receipt.Receipt.ExitCode))
	}
	...
	if err != nil {
		return cb(0, cid.Undef, xerrors.Errorf("WaitForPublishDeals getting deal info errored: %w", err))
	}

	return cb(res.DealID, receipt.Message, nil)
}
